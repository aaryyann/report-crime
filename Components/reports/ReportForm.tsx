...
export function ReportForm({ onComplete }: ReportFormProps) {
  ...
  const handeSubmit = async (e : React.FormEvent) => {
    ...
    try{
      ...
      if(!response.ok){
        throw new Error(result.error || "Failes to submit report")
      }
  ...
  return (
    <form onSubmit={handeSubmit} className="space-y-8">
      ...
      <div>
        ...
        <select
          ...
          className="w-full rounded-xl bg-zinc-900/50 border border-zinc-800 px-4 py-4
